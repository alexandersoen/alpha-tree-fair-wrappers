...
from fair_wrapper.datasets import FeaturePreprocessor, acs_age_dataset, cross_validate_dataset, german_credit_age_dataset, bank_age_dataset, preprocess, split_dataset
...
from fair_wrapper.sp_alphatree import SPAlphaTreeWrapper
from fair_wrapper.eoo_alphatree import make_empirical_EOOAlphaTreeWrapper, make_sklearn_EOOAlphaTreeWrapper
...
def main() -> None:
    ...
    parser.add_argument('--statistical-parity-mode', action='store_true', default=False,
                        help='train SP')
    parser.add_argument('--statistical-parity-soft-mode', action='store_true', default=False,
                        help='train SP Soft')
    parser.add_argument('--equality-of-opportunity-mode', action='store_true', default=False,
                        help='train EOO')
    parser.add_argument('--epsilon', type=float,
                        help='params for EOO ')
    parser.add_argument('-K', type=float,
                        help='params for EOO')
    parser.add_argument('--use-empirical-density', action='store_true',
                        help='params for EOO')
    parser.add_argument('--reverse', action='store_true', default=False,
                        help='reverse ordering of subgroups for either EOO or SP optimizations (default: False = increase mean posterior / positive classes)')
    parser.add_argument('--use-posterior-eoo', action='store_true', default=False,
                        help='use mean posterior in EOO')

    # Data selecting
    parser.add_argument('--dataset', type=str, default='acs', metavar='D',
                        help='selected dataset ["acs", "acs18", "german", "bank"] (default: "acs")')
    ...
    for cv_iter, (dataset_training, dataset_test) in tqdm_datasets:
        ...
        print('SKLEARN Score', dt.score(dataset_train.x, dataset_train.y.ravel()))
        ...
        if args.statistical_parity_mode:
            wrapper = SPAlphaTreeWrapper(blackbox_dt, loss, p_info, hypothesis_class, args.reverse)
        elif args.equality_of_opportunity_mode:
            if args.use_empirical_density:
                wrapper = make_empirical_EOOAlphaTreeWrapper(blackbox_dt, loss, p_info, hypothesis_class, args.epsilon, args.K, not args.use_posterior_eoo)
            else:
                wrapper = make_sklearn_EOOAlphaTreeWrapper(blackbox_dt, loss, p_info, hypothesis_class, args.epsilon, args.K, not args.use_posterior_eoo)
        else:
            wrapper = AlphaTreeWrapper(blackbox_dt, loss, p_info, hypothesis_class)
        ...
        cur_train_stats = [ get_stats(wrapper, dataset_train, 0) ]
        if not args.share_train_data:
            cur_alpha_stats = [ get_stats(wrapper, dataset_alpha, 0) ]
        cur_test_stats  = [ get_stats(wrapper, dataset_test, 0) ]
        ...
        dot = wrapper.twister.alpha_tree.basic_tree_dot(dataset_train.feature_names)
        print(dot.source)
        dot.render()
